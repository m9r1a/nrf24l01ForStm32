...
/* Private user code ---------------------------------------------------------*/
/* USER CODE BEGIN 0 */
 static uint8_t recData[32]={0};
 static uint8_t testPacket[5] = { 0x11, 0x22, 0x33, 0x44, 0x55 };
bool volatile dataReceived=false;
void ReceiveDataFromNRF(uint8_t* data, uint8_t pipe){
	memcpy(recData,data,32);
	dataReceived=true;
